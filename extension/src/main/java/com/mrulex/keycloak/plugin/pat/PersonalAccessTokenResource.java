...
import com.mrulex.keycloak.plugin.pat.dto.*;
...
public class PersonalAccessTokenResource {
    ...
    public Response listPats() {
        ...
        List<PatListItemDto> items = PatUtils.streamPats(user)
                .map(cred -> {
                    ...
                    return new PatListItemDto(cred.getId(), cred.getUserLabel(), created, data.expires(), data.roles());
                })
                .toList();
    ...
    public Response createPat(PatCreateRequestDto request) {
        ...
        CredentialModel model = PatUtils.buildCredential(request.name(), hash, request.roles(), request.expires());
        ...
        PatCreatedResponseDto response = new PatCreatedResponseDto(
                stored.getId(), stored.getUserLabel(), plaintext, created, request.expires(), request.roles());
    ...
    public Response listRoles() {
        ...
        List<PatRoleDto> roles = realm.getRolesStream()
    ...
    @GET
    @Path("auth")
    public Response auth(@HeaderParam("Authorization") String authHeader,
        ...
        String[] decoded = decodeBasic(authHeader);
        if (decoded == null) {
            return Response.status(Response.Status.UNAUTHORIZED).build();
        }
        ...
        if (requiredRole != null && !requiredRole.isBlank()) {
            if (!data.roles().contains(requiredRole)) {
