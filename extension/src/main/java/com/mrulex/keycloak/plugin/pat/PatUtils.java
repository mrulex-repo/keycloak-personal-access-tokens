...
public final class PatUtils {
    ...
    public static Argon2Function hashFunction() {
        return Argon2Function.getInstance(ARGON2_MEMORY, ARGON2_ITER, ARGON2_PARALLEL, ARGON2_LENGTH, Argon2.ID);
    }
    ...
    public static CredentialModel buildCredential(String name, String hash, List<String> roles, String expires) {
        try {
            Map<String, Object> secretData = new HashMap<>();
            secretData.put("hash", hash);

            Map<String, Object> credentialData = new HashMap<>();
            credentialData.put("roles", roles);
            if (expires != null && !expires.isBlank()) {
                credentialData.put("expires", expires);
            }
    ...
    public static PatCredentialData extractCredentialData(CredentialModel cred) {
        try {
            ...
            throw new RuntimeException("Failed to parse credentialData for credential " + cred.getId(), e);
        }
    }
    ...
    public static void validateTokenName(String name, UserModel user) {
        if (name == null || !TOKEN_NAME_PATTERN.matcher(name).matches()) {
            throw badRequest("name must be 1–64 characters, start with a letter, and contain only [a-z0-9_-]");
        }
        boolean duplicate = streamPats(user)
                .anyMatch(c -> name.equals(c.getUserLabel()));
        if (duplicate) {
    ...
    public static void recordFailedAttempt(KeycloakSession session, RealmModel realm, UserModel user) {
        BruteForceProtector protector = session.getProvider(BruteForceProtector.class);
        if (protector == null) return;
