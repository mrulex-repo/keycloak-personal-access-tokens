 ...
 *
 * Tests issue tokens via the OIDC Direct Grant (ROPC) flow using a PAT as the password,
 * against a real Keycloak 26 container. The {@code pat-test-client} in the test realm is
 * configured to use the {@code pat-direct-grant} flow (backed by {@code PatBasicAuthAuthenticator})
 * and has the {@code PatRolesTokenMapper} registered to filter JWT roles.
 */
...
class PatBasicAuthAuthenticatorTest {
    ...
    void validPat_jwtContainsPatRoles() throws Exception {
        ...
        Set<String> roles = decodeJwtRealmRoles(mapper.readTree(resp.body()).get("access_token").asText());
    ...
    /**
     * Issues a token via the OIDC Direct Grant (ROPC) flow using {@code pat-test-client}.
     * This client is bound to the {@code pat-direct-grant} flow, so {@code PatBasicAuthAuthenticator}
     * handles the authentication.
     */
    static HttpResponse<String> patDirectGrant(String username, String patToken) throws Exception {
        String body = "grant_type=password&client_id=" + PAT_CLIENT
                + "&username=" + username
                + "&password=" + patToken;
    ...
    static String getPasswordGrantToken(String clientId, String username, String password) throws Exception {
